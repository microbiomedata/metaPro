...
class GenMetadata:
    ...
    def write_to_json_file(self, filenames):
        ...
        with open(filenames[0], "w") as fptr1, open(filenames[1], "w") as fptr2:
            # dumping collection into metaproteomics_analysis json file.
            ...
            # dumping collection into data_object json file.
    ...
    def prepare_PeptideQuantification(self):
        ...
        with open(self.peptide_report, encoding="utf-8-sig") as tsvfile:
            ...
            for line in tsvreader:
                ...
                quant_dict["best_protein"] = line[3].replace(" ", "") # "nmdc:" + line[3].replace(" ", "")
                quant_dict["all_proteins"] = [
                    protein.replace(" ", "")
                    for protein in line[10].split(",")
                ]
                ...
                #     for protein in line[10].split(",")
                # ]
                quant_dict["min_q_value"] = float(line[12])
                quant_dict["peptide_spectral_count"] = int(line[13])
                quant_dict["peptide_sum_masic_abundance"] = int(float(line[14]))
                self.quant_bucket.append(quant_dict)
        pass
    ...
    def prepare_file_data_object_(self, file_path, file_name, description, activity_id):
        ...
        if checksum:
            ...
            self.data_object["url"] = (
                "https://nmdcdemo.emsl.pnnl.gov/proteomics/results/" + file_name
            )
    ...
    def create_has_output(self, activity_id):
        ...
        has_output.append(
            self.prepare_file_data_object_(
                ...
                "Aggregation of analysis tools{MSGFplus, MASIC} results", activity_id
            )
        )
        has_output.append(
            self.prepare_file_data_object_(
                ...
                "Aggregated peptides sequences from MSGF+ search results filtered to ~5% FDR", activity_id
            )
        )
        has_output.append(
            self.prepare_file_data_object_(
                ...
                "Aggregated protein lists from MSGF+ search results filtered to ~5% FDR", activity_id
            )
        )
        has_output.append(
            self.prepare_file_data_object_(
                ...
                "Overall statistics from MSGF+ search results filtered to ~5% FDR", activity_id
            )
        )
    ...
    def create_has_input(self, emsl_to_jgi_copy):
        ...
        has_input = []
        # add .RAW
        ptr_to_raw_in_bill_duncan_json = "emsl:output_" + self.dataset_id
        has_input.append(ptr_to_raw_in_bill_duncan_json)
        # add JGI fasta
    ...
    def prepare_activity(self, emsl_to_jgi_copy):
        """
        ...
        for dataset_id, values in emsl_to_jgi.items():
            if dataset_id not in [
                ...
                for genome_directory, locations in values["genome_directory"].items():
                    ...
                    meta_file.prepare_activity( emsl_to_jgi_copy)
