...
class GenMetadata:
    ...
    def write_to_json_file(self, filenames):
        ...
        with open(filenames[0], 'w') as fptr1 , open(filenames[1], 'w') as fptr2:
            ...
            for idx, activity_doc in enumerate(self.activity_coll.find()):
                activity_doc["id"] = activity_doc.pop("_id")
    ...
    def set_keys(self, dataset_id, genome_directory, annotation_file_name, resultant_file, fasta_file,
                                       contaminant_file, peptide_report, protein_report, qc_metric_report, started_at_time, ended_at_time):
...
if __name__ == '__main__':
    ...
    # setup the cursors
    coll_names = [f"{os.environ.get('STUDY')}_MetaProteomicAnalysis_activity",
        ...
        for dataset_id, values in emsl_to_jgi.items():
            if dataset_id not in ['contaminant_file_loc', 'analysis_activity_file_loc', 'data_objects_file_loc',
                ...
                for genome_directory, locations in values['genome_directory'].items():
                    ...
                    meta_file.set_keys(dataset_id, genome_directory, os.path.basename(locations['txt_faa_file_loc']),
                                       locations['resultant_file_loc'], locations['faa_file_loc'],
                                       contaminant_file, locations['peptide_report_loc'],
