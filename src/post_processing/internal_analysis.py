...
class InternalAnalysis:
    ...
    def process_data(self):
        ...
        data["SpecID"] = data.apply(
            lambda row: str(row["Dataset_ID"]) + "_" + str(row["Scan"]), axis=1
        )

        # Save protein to peptide mapping of forward peptide identification
        df_mapping = data[data["Protein_Type"] == "Forward"][
            ["Protein", "Peptide", "Clean Peptide Sequence"]
        ].copy()
        ...
        df_redundancy.rename(
            columns={"Protein": "Clean Peptide Sequence Redundancy"}, inplace=True
        )

        # merge redundancy to protein peptide map
        df_mapping = df_mapping.merge(
            df_redundancy, how="left", on=["Clean Peptide Sequence"]
        )
        del df_redundancy
        ...
        data.drop_duplicates(
            inplace=True
        )  # this is so important to remove any duplicated rows
        ...
        for i in range(len(dataset_list)):
            df_ff = data_cleaned_forward[
                data_cleaned_forward["Dataset_ID"] == dataset_list[i]
            ].copy()
            df_ff.to_csv(
                "Results/Data/"
                + str(dataset_list[i])
            ...
            df_rr = data_cleaned_reversed[
                data_cleaned_reversed["Dataset_ID"] == dataset_list[i]
            ].copy()
            df_rr.to_csv(
                "Results/Data/"
                + str(dataset_list[i])
...
def parameter_optimization(dataset_ID):
    ...
    """
    data_f = pd.read_csv(
        "Results/Data/" + str(dataset_ID) + "_forward_peptide_identification.csv"
    )
    data_r = pd.read_csv(
        "Results/Data/" + str(dataset_ID) + "_reversed_peptide_identification.csv"
    )
    ...
    def PepFDR(Params):
        ...
        df_r = data_r[
            (data_r["DelM_PPM"] < ppm_shift + delppm1)
            & (data_r["DelM_PPM"] > ppm_shift - delppm2)
            & (data_r["MSGFDB_SpecEValue"] < 10 ** log10_specprob)
        ].copy()
        df_f = data_f[
            (data_f["DelM_PPM"] < ppm_shift + delppm1)
            & (data_f["DelM_PPM"] > ppm_shift - delppm2)
            & (data_f["MSGFDB_SpecEValue"] < 10 ** log10_specprob)
        ].copy()
    ...
    # Miminize PepFDR
    initial_guess = [
        min(10, max(data_f["DelM_PPM"]) - ppm_shift),
        min(10, ppm_shift - min(data_f["DelM_PPM"])),
        -15,
    ]
    # print(dataset_ID, initial_guess)
    result = minimize(
        PepFDR, initial_guess, method="COBYLA", constraints=[con1, con2, con3]
    )
    if result.success:
        ...
        if result.success:
            ...
            print(
                "Failed optimization @ Dataset ID: %s, Database: %s" % (dataset_ID, DB)
            )
            print(initial_guess)
    ...
    axes[0].set_title(
        r"left bound: %.2f; right bound: %.2f"
        % (ppm_shift - fitted_params[1], ppm_shift + fitted_params[0])
    )
    # plot MSGFDB_SpecEValue
    sns.distplot(
        data_f["MSGFDB_SpecEValue"], label="forward", kde=False, ax=axes[1], bins=5000
    )
    sns.distplot(
        data_r["MSGFDB_SpecEValue"], label="reversed", kde=False, ax=axes[1], bins=5000
    )
    ...
    # #Filter the data
    df_f = data_f[
        (data_f["DelM_PPM"] < ppm_shift + fitted_params[0])
        & (data_f["DelM_PPM"] > ppm_shift - fitted_params[1])
        & (data_f["MSGFDB_SpecEValue"] < 10 ** fitted_params[2])
    ].copy()
    del data_f

    df_r = data_r[
        (data_r["DelM_PPM"] < ppm_shift + fitted_params[0])
        & (data_r["DelM_PPM"] > ppm_shift - fitted_params[1])
        & (data_r["MSGFDB_SpecEValue"] < 10 ** fitted_params[2])
    ].copy()
    del data_r

    # Calculate FDR
    f_spec = (
        df_f["SpecID"].unique().size
    )  # Modified on Apr23 to only count for unique dataset-scan
    ...
    df_Metadata = pd.DataFrame(
        {
            "PPM center": ppm_shift,
            "PPM cutoff left": ppm_shift - fitted_params[1],
            "PPM cutoff right": ppm_shift + fitted_params[0],
            "SpecProb cutoff": 10 ** fitted_params[2],
            "Spectra forward": f_spec,
            "Spectra reverse": r_spec,
            "Peptide forward": f_pep,
            "Peptide reverse": r_pep,
            "PeptideFDR": [fdr_pep],
            "SepctraFDR": [fdr_spec],
    ...
    df_SpectraCount = (
        df_spec.groupby(["Clean Peptide Sequence"])
        .size()
        .rename("Peptide Spectra Count")
        .reset_index()
    )
    del df_spec
    ...
    ######################################Spetra Count table##########################################
    df_Intensity = (
        df_f[["Clean Peptide Sequence", "StatMomentsArea"]]
        .groupby(["Clean Peptide Sequence"])
        .max()
    )
