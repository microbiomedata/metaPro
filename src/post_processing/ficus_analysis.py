...
class DataOutputtable:
    ...
    def get_FiltPeps(self, df):
        FiltPeps_df = self.FiltPeps(df)
        return FiltPeps_df[['PeptideSequence', 'Protein']].drop_duplicates().sort_values(by=['PeptideSequence'])
    ...
    def query_2(self):
        ...
        counted_peptides_df = grouped_by_protein['PeptideSequence'].count().reset_index(name="PeptideSequence_Count")
    ...
    def query_3(self):
        ...
        counted_protein_df = grouped_by_peptide['Protein'].count().reset_index(name="Protein_Count")
    ...
    def query_1(self):
        ...
        self._DataOutputtable_xlsx(FiltPeps_df.drop_duplicates().sort_values(by=['PeptideSequence']), 'query_1')
        return FiltPeps_df.drop_duplicates().sort_values(by=['PeptideSequence'])

    def query_4(self, table_1, table_2, table_3):

        # inner join on 'Protein'
        merge_1 = table_1.merge(table_2, how='inner',
                                on='Protein')
        # inner join on 'Protein'
        merge_2 = merge_1.merge(self.fasta_df, how='inner',
                                on='Protein')
        # inner join on 'PeptideSequence'
        merge_3 = merge_2.merge(table_3, how='inner',
                                on='PeptideSequence')

        col_of_interest = ['Protein', 'PeptideSequence', 'PeptideSequence_Count', 'Index', 'Protein_Count']

        self._DataOutputtable_xlsx(merge_3[col_of_interest].sort_values(by=['PeptideSequence']), 'query_4')
    ...
    def get_MaxPepCounts(self, table_4):
        ...
        peps_morethan_1_protein['MaxPeptideSequenceCount'] = peps_morethan_1_protein.groupby(['PeptideSequence'])[
            'PeptideSequence_Count'].transform(max)
        return peps_morethan_1_protein[['PeptideSequence', 'MaxPeptideSequenceCount']].drop_duplicates()

    def get_MaxPepCounts_table_4_joined(self, table_4):
        MaxPepCounts_df = self.get_MaxPepCounts(table_4)
        joined = MaxPepCounts_df.merge(table_4, how='inner', left_on=['PeptideSequence', 'MaxPeptideSequenceCount'],
                                       right_on=['PeptideSequence', 'PeptideSequence_Count'])
        joined['CountOfPeptideCounts'] = joined.groupby(['PeptideSequence'])['PeptideSequence_Count'].transform("count")
        return joined
    ...
    def get_BestSingleProteins(self, table_4):
        ...
        filtered['MaxPeptideSequenceCount'] = filtered.groupby(['PeptideSequence'])['PeptideSequence_Count'].transform(
            max)
        return filtered[['PeptideSequence', 'CountOfPeptideCounts', 'MaxPeptideSequenceCount']]

    def get_BestIndexedProtein(self, table_4):
        MaxPepCounts_table_4_joined_df = self.get_MaxPepCounts_table_4_joined(table_4)
        filtered = MaxPepCounts_table_4_joined_df[MaxPepCounts_table_4_joined_df['CountOfPeptideCounts'] > 1]
        filtered['MinIndexValue'] = filtered.groupby(['PeptideSequence'])['Index'].transform(min)
    ...
    def query_5(self, table_4):
        ...
        self._DataOutputtable_xlsx(peps_with_1_protein[['PeptideSequence', 'BestProtein']], 'query_5')
    ...
    def query_6(self, table_4):
        ...
        joined = BestSingleProteins_df.merge(table_4, how='inner',
                                             left_on=['PeptideSequence', 'MaxPeptideSequenceCount'],
                                             right_on=['PeptideSequence', 'PeptideSequence_Count'])
        joined.rename(columns={'Protein': 'BestProtein'}, inplace=True)

        self._DataOutputtable_xlsx(joined[['PeptideSequence', 'BestProtein']], 'query_6')
    ...
    def query_7(self, table_4):
        ...
        merged = BestIndexedProtein_df.merge(table_4, how='inner', left_on=['PeptideSequence', 'MinIndexValue'],
                                             right_on=['PeptideSequence', 'Index'])
        merged.rename(columns={'Protein': 'BestProtein'}, inplace=True)

        self._DataOutputtable_xlsx(merged[['PeptideSequence', 'BestProtein']].drop_duplicates(), 'query_7')
    ...
    def query_0(self):
        ...
        attributeTag_df['ec_number'] = attributeTag_df['ec_number']  # .str.replace('EC:', '')
    ...
    def query_8(self):
        ...
        merged = table_5_6_7_unioned.merge(AnnotationSplitout_df, how='left', left_on=['BestProtein'],
                                           right_on=['Protein'])
        col_of_interest = ['PeptideSequence', 'BestProtein', 'Product', 'ec_number', 'pfam', 'ko', 'cog']

        cols_to_rename = {'ec_number': 'EC_Number',
                          'ko': 'KO',
                          'cog': 'COG'}
    ...
    def get_FwdPeptideSequences(self):
        ...
        FiltPeps_df['GeneCount'] = FiltPeps_df.groupby(['PeptideSequence'])['Protein'].transform('nunique')
    ...
    def query_9(self):
        ...
        FiltPassPeptideAbundanceData = self.get_FiltPeps_gen(temp_df)[['PeptideSequence', 'Protein']]

        merged = FwdPeptideSequences_df.merge(FiltPassPeptideAbundanceData, how='inner', left_on=['PeptideSequence'],
                                              right_on=['PeptideSequence'])

        self._DataOutputtable_xlsx(
            merged[['PeptideSequence', 'GeneCount', 'Protein']].drop_duplicates().sort_values(by=['PeptideSequence']),
            'query_9')
        return merged[['PeptideSequence', 'GeneCount', 'Protein']].drop_duplicates().sort_values(by=['PeptideSequence'])
    ...
    def query_10(self):
        ...
        AnnotationSplitout_df = self.query_0()
        merged = table_9.merge(AnnotationSplitout_df, how='left', left_on=['Protein'],
                               right_on=['Protein'])
        merged['annotation'] = merged.apply(
            lambda x: self.build_annotation_str(x.Protein, x.Product, x.pfam, x.ko, x.ec_number, x.cog), axis=1)

        self._DataOutputtable_xlsx(
            merged[['PeptideSequence', 'GeneCount', 'Protein', 'annotation']].sort_values(by=['PeptideSequence']),
            'query_10')
        return merged[['PeptideSequence', 'GeneCount', 'Protein', 'annotation']].sort_values(by=['PeptideSequence'])
    ...
    def query_11(self):
        ...
        table_10['FullGeneList'] = table_10.sort_values('Protein').groupby(['PeptideSequence', 'GeneCount'])[
            'Protein'].transform(lambda x: ', '.join(x))

        # sort them!
        get_list_func = lambda x: x.split(', ') if len(x.split(', ')) > 1 else x
        f = lambda x: ', '.join(sorted(get_list_func(x))) if isinstance(get_list_func(x), list) else get_list_func(x)
        table_10['FullGeneList'] = table_10['FullGeneList'].apply(f)

        table_10['AnnotationList'] = table_10.sort_values('annotation').groupby(['PeptideSequence', 'GeneCount'])[
            'annotation'].transform(lambda x: ' | '.join(x))

        self._DataOutputtable_xlsx(table_10[
            ['PeptideSequence', 'GeneCount', 'FullGeneList', 'AnnotationList']].drop_duplicates().sort_values(
            by=['PeptideSequence']), 'query_11')
        return table_10[
            ['PeptideSequence', 'GeneCount', 'FullGeneList', 'AnnotationList']].drop_duplicates().sort_values(
            by=['PeptideSequence'])
    ...
    def query_12(self):
        ...
        FiltPassPeptideAbundanceData = self.get_FiltPeps_gen(temp_df)[
            ['SpecIndex', 'PeptideSequence', 'StatMomentsArea']].drop_duplicates(
            subset=['SpecIndex', 'PeptideSequence', 'StatMomentsArea'])  # .sort_values(by=[''])
        FiltPassPeptideAbundanceData['SpectralCount'] = FiltPassPeptideAbundanceData.groupby(['PeptideSequence'])[
            'SpecIndex'].transform('count')
        FiltPassPeptideAbundanceData["sum(StatMomentsArea)"] = \
            FiltPassPeptideAbundanceData.groupby(['PeptideSequence'])['StatMomentsArea'].transform('sum')

        self._DataOutputtable_xlsx(FiltPassPeptideAbundanceData[
            ['PeptideSequence', 'SpectralCount', "sum(StatMomentsArea)"]].drop_duplicates().sort_values(
            by=['PeptideSequence']), 'query_12')
        return FiltPassPeptideAbundanceData[
            ['PeptideSequence', 'SpectralCount', "sum(StatMomentsArea)"]].drop_duplicates().sort_values(
            by=['PeptideSequence'])

    def query_13(self):
        temp_df = self.resultant_df.copy()
        filtered = self.get_FiltPeps_gen(temp_df)[['Dataset_x', 'PeptideSequence', 'QValue']]
        groupby_columns = ['Dataset_x', 'PeptideSequence']
        filtered['min(QValue)'] = filtered.groupby(groupby_columns)['QValue'].transform('min')
        PeptideGeneListsANDAnnotationList_df = self.query_11()

        # inner join on 'PeptideSequence'
        merged_1 = filtered.merge(PeptideGeneListsANDAnnotationList_df, how='inner', left_on=['PeptideSequence'],
                                  right_on=['PeptideSequence'])
        ...
        merged_2 = merged_1.merge(PeptideBestProteinAnnotated_df, how='left', on=['PeptideSequence'])

        PeptideAbundanceInfo_df = self.query_12()
        # inner join on 'PeptideSequence'
        merged_3 = merged_2.merge(PeptideAbundanceInfo_df, how='inner', left_on=['PeptideSequence'],
                                  right_on=['PeptideSequence'])

        cols_to_rename = {'Dataset_x': 'DatasetName', 'sum(StatMomentsArea)': 'sum(MASICAbundance)'}
        merged_3.rename(columns=cols_to_rename, inplace=True)

        self._DataOutputtable_xlsx(
            merged_3[['DatasetName', 'PeptideSequence', 'BestProtein', 'Product', 'EC_Number', 'pfam', 'KO',
                      'COG', 'GeneCount', 'FullGeneList', 'AnnotationList', 'min(QValue)', 'SpectralCount',
                      'sum(MASICAbundance)']].drop_duplicates().sort_values(by=['DatasetName']), 'query_13')
        return merged_3[['DatasetName', 'PeptideSequence', 'BestProtein', 'Product', 'EC_Number', 'pfam', 'KO',
                         'COG', 'GeneCount', 'FullGeneList', 'AnnotationList', 'min(QValue)', 'SpectralCount',
                         'sum(MASICAbundance)']].drop_duplicates().sort_values(by=['DatasetName'])

    def query_14(self):
        Peptide_Report = self.query_13()

        Peptide_Report['UniquePeptideCount'] = Peptide_Report.groupby(['BestProtein'])['PeptideSequence'].transform(
            'count')
        Peptide_Report['SummedSpectraCounts'] = Peptide_Report.groupby(['BestProtein'])['SpectralCount'].transform(
            'sum')
        Peptide_Report['SummedPeptideMASICAbundances'] = Peptide_Report.groupby(['BestProtein'])[
            'sum(MASICAbundance)'].transform('sum')

        self._DataOutputtable_xlsx(Peptide_Report[['BestProtein', 'UniquePeptideCount', 'SummedSpectraCounts',
                                                   'SummedPeptideMASICAbundances']].drop_duplicates().sort_values(
            by=['BestProtein']), 'query_14')
        return Peptide_Report[['BestProtein', 'UniquePeptideCount', 'SummedSpectraCounts',
                               'SummedPeptideMASICAbundances']].drop_duplicates().sort_values(by=['BestProtein'])
    ...
    def query_15(self):
        ...
        merged = PeptideBestProteinAnnotated_df.merge(temp_df,
                                                      how='inner', left_on=['PeptideSequence'],
                                                      right_on=['PeptideSequence'])
        non_decoy_filtered_df = merged[~merged['Protein'].str.startswith('XXX')]
        self._DataOutputtable_xlsx(non_decoy_filtered_df[['BestProtein', 'Protein']].drop_duplicates(
            subset=['BestProtein', 'Protein']).sort_values(by=['BestProtein']), 'query_15')
        return non_decoy_filtered_df[['BestProtein', 'Protein']].drop_duplicates(
            subset=['BestProtein', 'Protein']).sort_values(by=['BestProtein'])

    def query_16(self):
        BestProteinGeneLists = self.query_15()  # .drop_duplicates(subset=['Protein'])

        # prepare protein list
        BestProteinGeneLists['FullGeneList'] = BestProteinGeneLists.groupby(['BestProtein'])['Protein'].transform(
            lambda x: ', '.join(x))
        # sort them!
        get_list_func = lambda x: x.split(', ') if len(x.split(', ')) > 1 else x
        f = lambda x: ', '.join(sorted(get_list_func(x))) if isinstance(get_list_func(x), list) else get_list_func(x)
        BestProteinGeneLists['FullGeneList'] = BestProteinGeneLists['FullGeneList'].apply(f)

        BestProteinGeneLists['GeneCount'] = BestProteinGeneLists.groupby(['BestProtein'])['Protein'].transform('count')
        BestProteinGeneLists.sort_values(['FullGeneList'], ascending=True, inplace=True)

        self._DataOutputtable_xlsx(
            BestProteinGeneLists[['BestProtein', 'GeneCount', 'FullGeneList']].drop_duplicates().sort_values(
                by=['BestProtein']), 'query_16')
        return BestProteinGeneLists[['BestProtein', 'GeneCount', 'FullGeneList']].drop_duplicates().sort_values(
            by=['BestProtein'])

    def mod_build_annotation_str(self, BestProtein, Protein, Product, pfam, ko, ec_number, cog):
    ...
    def query_17(self):
        ...
        ProteinBestProteinsAnnotated = table_15.merge(AnnotationSplitout_df, how='left', left_on=['Protein'],
                                                      right_on=['Protein'])
        #         display(ProteinBestProteinsAnnotated)
        ProteinBestProteinsAnnotated['annotation'] = ProteinBestProteinsAnnotated.apply(
            lambda x: self.mod_build_annotation_str(x.BestProtein, x.Protein, x.Product, x.pfam, x.ko, x.ec_number,
                                                    x.cog), axis=1)

        # display(ProteinBestProteinsAnnotated)
        self._DataOutputtable_xlsx(
            ProteinBestProteinsAnnotated[['BestProtein', 'Protein', 'annotation']].drop_duplicates().sort_values(
                by=['BestProtein']), 'query_17')

        return ProteinBestProteinsAnnotated[['BestProtein', 'Protein', 'annotation']].drop_duplicates().sort_values(
            by=['BestProtein'])
    ...
    def query_18(self):
        ...
        BestProteinAnnotationLists['AnnotationList'] = BestProteinAnnotationLists.groupby(['BestProtein'])[
            'annotation'].transform(lambda x: ' | '.join(x))
        self._DataOutputtable_xlsx(
            BestProteinAnnotationLists[['BestProtein', 'AnnotationList']].drop_duplicates().sort_values(
                by=['BestProtein']), 'query_18')
        return BestProteinAnnotationLists[['BestProtein', 'AnnotationList']].drop_duplicates().sort_values(
            by=['BestProtein'])

    def query_19(self):
        Peptide_Report_df = self.peptide_report.copy()[['DatasetName', 'BestProtein', 'Product', 'EC_Number', 'pfam', 'KO', 'COG']]
        BestProteinGeneLists = self.query_16()
        merged_1 = Peptide_Report_df.merge(BestProteinGeneLists, how='inner', left_on=['BestProtein'],
                                           right_on=['BestProtein'])

        BestProteinAnnotationLists = self.query_18()
        merged_2 = merged_1.merge(BestProteinAnnotationLists, how='inner', left_on=['BestProtein'],
                                  right_on=['BestProtein'])

        PeptideAbundanceInfo_df = self.query_14()
        merged_3 = merged_2.merge(PeptideAbundanceInfo_df, how='inner', left_on=['BestProtein'],
                                  right_on=['BestProtein'])

        distinct_colm = ['DatasetName', 'BestProtein', 'Product', 'EC_Number', 'pfam', 'KO',
                         'COG', 'GeneCount', 'FullGeneList', 'AnnotationList', 'UniquePeptideCount',
                         'SummedSpectraCounts', 'SummedPeptideMASICAbundances']
        self._DataOutputtable_xlsx(
            merged_3[distinct_colm].drop_duplicates(subset=distinct_colm).sort_values(by=['DatasetName']), 'query_19')
        return merged_3[distinct_colm].drop_duplicates(subset=distinct_colm).sort_values(by=['DatasetName'])

    def query_20(self):

        total_protein_count = pd.DataFrame({'total_protein_count': self.fasta_df.Protein.nunique()}, index=[0])
    ...
    def query_21(self):
        ...
        PSM_Count = pd.DataFrame({'PSM_Count': len(filtered.groupby(['Scan', 'Charge'])['Scan'])}, index=[0])
    ...
    def query_22(self):
        ...
        Total_PSM_Count = pd.DataFrame(
            {'Total_PSM_Count': len(temp_df[['Scan', 'Charge']].groupby(['Scan', 'Charge'])['Scan'])}, index=[0])
        self._DataOutputtable_xlsx(Total_PSM_Count, 'query_22')
        return Total_PSM_Count

    def query_23(self):
        Table12 = pd.DataFrame(
            {'PSM_identification_rate': (self.query_21().PSM_Count / self.query_22().Total_PSM_Count) * 100}, index=[0])
    ...
    def query_24(self):
        ...
        unique_peptide_seq_count = Peptide_Report_df.PeptideSequence.drop_duplicates().count()
        ...
        qc_metrics = pd.DataFrame({'total_protein_count': self.query_20().total_protein_count,
                                   ...
                                   'mean_peptide_count': mean_peptide_count}, index=[0])
