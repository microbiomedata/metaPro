...
def register_job_in_emsl_to_jgi(
    dataset_id, genome_directory, key, value, emsl_to_jgi_copy
):
            ...
            if dataset_id not in [
                ...
                for genome_directory, locations in values["genome_directory"].items():
                    ...
                    data_obj = DataOutputtable(
                        ...
                        resultant_file,
                        fasta_txt_file,
                        os.environ.get("QVALUE_THRESHOLD"),
                        dataset_id,
                        genome_directory,
                    )
                    (
                    ...
                    save_job_results = os.path.join(
                        result_loc, dataset_id, genome_directory
                    )
                    save_at = os.path.join(save_job_results, "reports")
                    if not os.path.exists(save_at):
                        os.makedirs(save_at)

                    pep_rpt_file = os.path.join(
                        save_at, f"{dataset_id}_{genome_directory}_Peptide_Report.tsv"
                    )
                    register_job_in_emsl_to_jgi(
                        dataset_id,
                        genome_directory,
                    ...
                    pro_rpt_file = os.path.join(
                        save_at, f"{dataset_id}_{genome_directory}_Protein_Report.tsv"
                    )
                    register_job_in_emsl_to_jgi(
                        dataset_id,
                        genome_directory,
                    ...
                    qc_m_rpt_file = os.path.join(
                        save_at, f"{dataset_id}_{genome_directory}_QC_metrics.tsv"
                    )
                    register_job_in_emsl_to_jgi(
                        dataset_id,
                        genome_directory,
                        "qc_metric_report_loc",
                        qc_m_rpt_file,
                        emsl_to_jgi_copy,
                    )
