    ...
    def collected(*args, **kw):
        ...
        if method.__name__ == "analysis_job":
            if 'total_runtime' not in ANALYSIS_JOBS_OBJECT:
                ANALYSIS_JOBS_OBJECT['total_runtime']={'runtime'        : time.strftime("%H:%M:%S" , time.gmtime((te - ts))),
                                                       'current_m/m(MB)': current / 10 ** 6,
                                                       'peak_m/m(MB)'   : peak / 10 ** 6}
        else:
            if method.__name__ not in ANALYSIS_JOBS_OBJECT:
                ANALYSIS_JOBS_OBJECT[method.__name__] = { 'runtime'        : time.strftime("%H:%M:%S", time.gmtime((te - ts))),
                                                          'current_m/m(MB)': current / 10 ** 6,
                                                          'peak_m/m(MB)'   : peak / 10 ** 6}
        tracemalloc.stop()
        return result
...
class ProcessingTools:
    ...
    def execute_masic(self, raw_file):
        ...
        if not os.path.isfile(output_file):
            if os.system( f"mono /app/masic/MASIC_Console.exe \
                            /I:{raw_file} \
                            /O:{save_at} \
                            ...
                            | tee -a { os.path.join( self.log_collected_at,'0_masic.log') }" ) != 0:
                raise
        else:
    ...
    def execute_msconvert(self, raw_file):
        ...
        if not os.path.isfile(output_file):
            if os.system(f"wine msconvert \
                                {raw_file} \
                                --zlib \
                                --filter 'peakPicking true 2-' \
                                -o {save_at}\
                                --verbose | tee -a {os.path.join( self.log_collected_at, '1_MSconvert.log')}") != 0:
                raise
        else:
            print(f"Already exists :: {output_file}")
        return output_file
    ...
    def execute_msgfplus(self, mzml_file, fasta_file):
        ...
        if not os.path.isfile(output_file):
            if os.system(f"java -Xmx32G -jar /app/msgf/MSGFPlus.jar \
                            -s {mzml_file} \
                            -o {output_file} \
                            -d {fasta_file} \
                            -thread 16 \
                            -conf {self.MSGFPLUS_PARAM_FILE} \
                            -verbose 1 | tee -a {os.path.join( self.log_collected_at, '2_MSGFPlus.log')}") != 0:
                raise
        ...
        revcat_fasta= os.path.join(save_at, 'fasta_residuals', f"{faa_name}.revCat.fasta")
    ...
    def execute_mzid2tsv(self, mzid_file):
        ...
        if not os.path.isfile(output_file):
            if os.system(f"mono /app/mzid2tsv/net462/MzidToTsvConverter.exe \
                            -mzid:{mzid_file} \
                            -tsv:{output_file} \
                            -unroll \
                            -showDecoy | tee -a {os.path.join( self.log_collected_at, '3_MzidToTsvConverter.log')}") != 0:
                raise
        else:
            print(f"Already exists :: {output_file}")
        return output_file
...
`       -T:MassCorrectionTagsFilePath
        ...
        if not os.path.isfile(output_file):
            if os.system(f"mono /app/phrp/PeptideHitResultsProcRunner.exe \
                            -I:{tsv_file} \
                            -O:{save_at} \
                            ...
                            -ProteinMods \
                            -F:{revcat_faa_file} \
                            | tee -a {os.path.join( self.log_collected_at, '4_TsvToSynConverter.log')}") != 0:
                raise
        else:
            print(f"Already exists :: {output_file}")
    ...
    def execute_protein_digestion_simulator(self, fasta_file):
        ...
        output_file = os.path.join(save_at, os.path.splitext( (os.path.basename(fasta_file)) )[0] + ".txt")
        if not os.path.isfile(output_file):
            if os.system(f"wine /app/ProteinDigestionSimulator/ProteinDigestionSimulator.exe \
                                -I:{fasta_file} \
                                -O:{save_at} \
                                -F    \
    ...
    def analysis_job(self, dataset_id, faa_file_loc, raw_file_loc ):
        ...
        try:
            ...
            try:
                mzid_file, revcat_faa_file = self.execute_msgfplus(mzml_file, faa_file_loc)
                try:
                    tsv_file = self.execute_mzid2tsv(mzid_file)
                    try:
                        self.execute_tsv2syn(tsv_file, revcat_faa_file)
                        print(f"Processing ended {dataset_id} : {os.path.basename(faa_file_loc)}")

                    except Exception as e:
                        logger.error(f"PeptideHitResultsProcRunner failed for {dataset_id}", exc_info=True)
                except Exception as e:
                    logger.error(f"MzidToTsvConverter failed for {dataset_id}", exc_info=True)
            except Exception as e:
    ...
    def run_n_log_job(self, dataset_id, genome_directory, faa_file_loc, raw_file_loc,emsl_to_jgi_copy ):
    ...
    def convert_faa2txt(self,dataset_id, faa_file_loc):
        ...
        try:
            ...
            logger.error(f"ProteinDigestionSimulator failed for {dataset_id}", exc_info=True)
        return faa_txt
    ...
    def merge_analysis_jobs(self, dataset_id, genome_directory):
        ...
        output_file = os.path.join(save_at, f"{dataset_id}_{genome_directory}_MSGFjobs_MASIC_resultant.tsv")
        if not os.path.isfile(output_file):
            try:
    ...
    def contaminate_fasta(self, files):
        ...
        save_at = os.path.join(self.save_job_results, 'msgfplus_output', 'fasta_residuals')
        if not os.path.exists(save_at):
            os.makedirs(save_at)
            ...
            for dataset_id, values in emsl_to_jgi.items():
                if dataset_id not in ['contaminant_file_loc', 'analysis_activity_file_loc', 'data_objects_file_loc',
                    ...
                    for genome_directory, locations in values['genome_directory'].items():
                        ...
                        # create log_dir
                        self.save_job_results = os.path.join(self.result_loc, dataset_id, genome_directory)
                        self.log_collected_at= os.path.join( os.path.abspath( self.save_job_results) , 'analysis_jobs_logs')
...
if __name__ == '__main__':
    ...
    # and log whether you're using modefications.
