...
class MetaproInputFile():
    ...
    def download_files(self) -> List[DownloadedFile]:
        ...
        for do in self.download_list:
            dat = {}
            path = self.settings_dir if do[0] == "settings" else self.datafiles_dir
            ...
            if is_sucess:
                if self.should_check_md5:
                    fp = path / do[1]["name"]
                    file_md5 = self.get_md5(fp)
...
class MetaproInput(ABC):
    def __init__(self, is_matched_metagenome, study_id, masic_param_id, 
                 msgf_param_id, contam_id, q_value_threshold, 
                 execution_resource, output_dir, data_url):
...
@click.command()
@click.option('--study-id', prompt='Your study id', help='The study id to get biosample IDs from.')
@click.option('--output-dir', prompt='Output directory', help='The absolute output directory to write downloaded files to.')
@click.option('-dl', is_flag=True, default=False, help='Set to download files.')
def main(study_id, output_dir, dl):
    ...
    matched_metagenome_input = MatchedMetagenomeInput(
        study_id,
        masic_param_id,
        msgf_param_id,
        contam_id,
